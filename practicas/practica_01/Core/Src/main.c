...
int main(void) {
	...
	uint32_t time_togle = 200; // start at 200 ms 50% dt
	uint8_t last_btn_state = 1; // state button (1 = no press, pull-up)
...
//		HAL_Delay(200);
		...
		HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin);
		HAL_Delay(time_togle);

		uint8_t curr_btn_state = HAL_GPIO_ReadPin(B1_GPIO_Port, B1_Pin);

		if (last_btn_state == 1 && curr_btn_state == 0) {
			// detect flanc low
			time_togle = (time_togle == 200) ? 500 : 200;
		}
